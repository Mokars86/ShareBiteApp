...
const FoodCard: React.FC<{ item: FoodItem, onClick: () => void }> = ({ item, onClick }) => {
  ...
  return (
    <div onClick={onClick} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-4 cursor-pointer active:scale-[0.99] transition">
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mt-1 mb-2">
            ...
            <span className="text-xs text-gray-600">{item.donorName} • ⭐ {item.donorRating}</span>
        </div>
...
const Onboarding: React.FC<{ onFinish: () => void }> = ({ onFinish }) => {
  ...
  return (
    <div className="h-full flex flex-col justify-center items-center p-8 bg-white text-center animate-fade-in">
      <div className="flex-1 flex flex-col justify-center items-center">
        <div className="mb-8 animate-bounce">{slides[slide].icon}</div>
        <h1 className="text-2xl font-bold text-gray-800 mb-2">{slides[slide].title}</h1>
        <p className="text-gray-500 max-w-xs">{slides[slide].text}</p>
      </div>
      <div className="w-full">
        <div className="flex justify-center gap-2 mb-8">
            {slides.map((_, i) => (
                <div key={i} className={`h-2 rounded-full transition-all duration-300 ${slide === i ? 'w-8 bg-eco-600' : 'w-2 bg-gray-300'}`} />
            ))}
        </div>
        <button 
          onClick={() => {
          ...
          className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold text-lg shadow-xl"
        >
...
const AuthScreen: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
    return (
        <div className="h-full bg-white p-8 flex flex-col justify-center animate-fade-in">
            <div className="mb-10">
                <div className="inline-block p-3 rounded-2xl bg-eco-50 mb-4">
                  <Leaf className="w-8 h-8 text-eco-600" />
                </div>
                <h1 className="text-3xl font-bold text-eco-700 mb-2">ShareBite</h1>
                <p className="text-gray-500">Welcome back! Let's save some food.</p>
            </div>

            <div className="space-y-4">
                <input type="email" placeholder="Email Address" className="w-full p-4 bg-gray-50 rounded-xl border border-gray-100 outline-none focus:ring-2 focus:ring-eco-500" />
                <input type="password" placeholder="Password" className="w-full p-4 bg-gray-50 rounded-xl border border-gray-100 outline-none focus:ring-2 focus:ring-eco-500" />
...
const DetailView: React.FC<{ item: FoodItem, onBack: () => void, onClaim: () => void }> = ({ item, onBack, onClaim }) => {
    return (
        <div className="h-full flex flex-col bg-white pb-20 overflow-y-auto animate-fade-in">
            <div className="relative h-72 w-full">
                <img src={item.imageUrl} className="w-full h-full object-cover" />
                <button onClick={onBack} className="absolute top-4 left-4 bg-white/50 backdrop-blur p-2 rounded-full hover:bg-white transition">
            ...
            <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="bg-green-50 text-eco-700 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap">{item.status}</span>
                </div>

                <div className="flex items-center gap-3 my-4 p-3 bg-gray-50 rounded-xl">
                    <img src={item.donorAvatar} className="w-10 h-10 rounded-full" />
                    <div className="flex-1">
                        <p className="font-bold text-sm">{item.donorName}</p>
                        <p className="text-xs text-gray-500">Eco Hero • {item.donorRating} ⭐</p>
                    </div>
                    <button className="bg-white p-2 rounded-full border shadow-sm hover:bg-gray-50">
                ...
                <div className="space-y-4">
                    ...
                    <div className="border-t pt-4 grid grid-cols-2 gap-4">
                         ...
                         <div>
                             ...
                             <p className="text-sm font-medium text-gray-800">{item.distance} km away</p>
                         </div>
                    </div>
                </div>
            </div>
        ...
        <div className="h-full bg-gray-50 flex flex-col animate-fade-in">
            ...
            <div className="bg-white p-6 pb-4 rounded-b-3xl shadow-sm z-10">
                <div className="flex flex-col items-center">
                    <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-eco-50 mb-3 relative shadow-md">
                        <img src={user.avatar} className="w-full h-full object-cover" alt="Profile" />
                        <div className="absolute bottom-0 right-0 bg-eco-600 text-white text-xs px-2 py-0.5 rounded-full border border-white">
                            {user.rating} ★
                        </div>
                    </div>
                    <h2 className="text-xl font-bold text-gray-900">{user.name}</h2>
                    <p className="text-sm text-gray-500 mb-4">{user.role === 'BOTH' ? 'Donor & Recipient' : user.role}</p>

                    {/* Role Switcher */}
                    {user.role === 'BOTH' && (
                        <div className="flex bg-gray-100 p-1 rounded-full w-64 relative">
                             <button 
                                onClick={() => setViewMode('DONOR')}
                                className={`flex-1 py-2 rounded-full text-sm font-semibold transition-all duration-300 z-10 ${viewMode === 'DONOR' ? 'bg-white shadow-md text-eco-700' : 'text-gray-500'}`}
                             >
                                Donor
                             </button>
                             <button 
                                onClick={() => setViewMode('RECIPIENT')}
                                className={`flex-1 py-2 rounded-full text-sm font-semibold transition-all duration-300 z-10 ${viewMode === 'RECIPIENT' ? 'bg-white shadow-md text-warm-600' : 'text-gray-500'}`}
                             >
                                Recipient
                             </button>
                        </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 pb-24 space-y-6">
                    ...
                    <div className="space-y-6 animate-fade-in">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="text-xs text-eco-600 font-semibold cursor-pointer">View All</span>
                            </div>
                                ...
                                <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
                                    {user.badges.map((badge, i) => (
                                        <div key={i} className="flex flex-col items-center min-w-[80px]">
                                            ...
                                            <span className="text-xs text-center font-medium text-gray-600 leading-tight">{badge}</span>
                                        </div>
                                    ))}
                                    <div className="flex flex-col items-center min-w-[80px] opacity-50 grayscale">
                                        <div className="bg-gray-100 text-gray-400 p-3 rounded-full mb-2">
                                            <Award className="w-6 h-6" />
                                        </div>
                        ...
                        {/* Impact Summary */}
                        <div className="bg-gradient-to-br from-eco-50 to-white p-5 rounded-2xl border border-eco-100 shadow-sm">
                            <h3 className="font-bold text-gray-800 mb-2">Community Impact</h3>
                            <p className="text-sm text-gray-600">You've helped 8 neighbors this month! Keep up the great work.</p>
                        </div>
                    </div>
                ) : (
                    <div className="space-y-6 animate-fade-in">
                        ...
                        {/* Recent Pickups Mock */}
                        <div>
                            <h3 className="font-bold text-gray-800 mb-3">Recent Pickups</h3>
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 divide-y">
                                {[1, 2].map((_, i) => (
                                    <div key={i} className="p-4 flex items-center gap-3">
                                        ...
                                        <div className="flex-1">
                                            <p className="font-medium text-gray-900 text-sm">Bagels & Pastries</p>
                                            <p className="text-xs text-gray-500">From Bakery Delight • 2 days ago</p>
                                        </div>
                ...
                <div className="pt-4 space-y-3">
                     <button className="w-full flex items-center justify-between p-4 bg-white rounded-xl shadow-sm border border-gray-100 text-gray-700 hover:bg-gray-50 transition active:scale-[0.99]">
                     ...
                     <button 
                        ...
                        className="w-full flex items-center justify-center gap-2 p-4 text-red-500 font-medium bg-red-50 hover:bg-red-100 rounded-xl transition active:scale-[0.99]"
                     >
...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-gray-50 relative shadow-2xl overflow-hidden">
      ...
      {/* Header (Only on Home) */}
      {activeTab === 'HOME' && (
        <header className="bg-white p-4 pt-6 pb-2 shadow-sm z-10 sticky top-0">
          <div className="flex justify-between items-center mb-4">
            <div>
                <h1 className="text-xl font-extrabold text-eco-600 flex items-center gap-2">
            ...
            <div className="bg-gray-100 p-2 rounded-full cursor-pointer hover:bg-gray-200" onClick={() => setActiveTab('PROFILE')}>
          ...
          <div className="flex gap-2 items-center mb-3">
             <div className="flex-1 bg-gray-100 rounded-lg flex items-center px-3 py-2">
                ...
                <input placeholder="Search food..." className="bg-transparent text-sm w-full outline-none" />
             </div>
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg">
             <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-md text-sm font-medium transition ${homeMode === 'LIST' ? 'bg-white shadow text-eco-600' : 'text-gray-500'}`}
             >
                <List className="w-4 h-4" /> Feed
             </button>
             <button 
                onClick={() => setHomeMode('MAP')}
                className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-md text-sm font-medium transition ${homeMode === 'MAP' ? 'bg-white shadow text-eco-600' : 'text-gray-500'}`}
             >
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar pb-1">
             {['All', 'Veg', 'Halal', 'Bakery', 'Homemade'].map(f => (
                 <button 
                    ...
                    className={`px-4 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition ${filterType === f ? 'bg-eco-600 text-white' : 'bg-gray-100 text-gray-600'}`}
                 >
                    {f}
                 </button>
             ))}
      ...
      <main className="flex-1 overflow-y-auto relative bg-gray-50">
                      ...
                      <div className="h-full flex flex-col">
                          ...
                          {chatSessions.length === 0 ? (
                              <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                                  ...
                                  <p className="font-medium text-gray-600 mb-1">No active chats</p>
                              ...
                              <div className="divide-y divide-gray-100">
                                  {chatSessions.map(session => (
                                      <div 
                                        ...
                                        className="p-4 flex items-center gap-3 hover:bg-gray-50 transition cursor-pointer"
                                      >
          ...
          {activeTab === 'PROFILE' && (
              <ProfileScreen user={CURRENT_USER} onLogout={handleLogout} />
          )}
      ...
      {/* Bottom Navigation */}
      <nav className="bg-white border-t border-gray-100 p-2 pb-6 flex justify-around items-center absolute bottom-0 w-full z-20">
          <button 
            onClick={() => setActiveTab('HOME')}
            className={`flex flex-col items-center p-2 rounded-lg transition ${activeTab === 'HOME' ? 'text-eco-600' : 'text-gray-400'}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center p-2 rounded-lg transition ${activeTab === 'CHAT' ? 'text-eco-600' : 'text-gray-400'}`}
          >
              <div className="relative">
          ...
          <button 
            ...
            className={`flex flex-col items-center p-2 rounded-lg transition ${activeTab === 'PROFILE' ? 'text-eco-600' : 'text-gray-400'}`}
          >
